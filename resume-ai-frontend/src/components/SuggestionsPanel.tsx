...
export type AIAnalysis = {
  ...
  jobRecommendations?: { title: string; description: string }[];
};
    ...
    <div>
      <p className="text-gray-700 dark:text-gray-300 text-sm">
        ...
        <span className="font-semibold">
          {allMatched.length + allMissing.length}
        </span>{" "}
        prioritized JD terms{" "}
        <span className="font-semibold">({matchPercentage}%)</span>.
      </p>
      {allMissing.length > 0 && (
        <p className="text-gray-700 dark:text-gray-300 text-sm">
          Consider adding the missing skills/keywords in your summary or skills
          section.
        </p>
      )}
        ...
        <>
          {insightMain && (
            <div className="mt-2 text-sm text-gray-700 dark:text-gray-300 space-y-2">
              <p className="font-bold text-justify">Insight Summary:</p>
              {insightMain
                .split(/\n{2,}|\r\n{2,}/)
                .map((paragraph, idx) => (
              ...
              <div className="mt-2 space-y-2">
                {recommendations.map((rec, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <div className="w-2 flex-shrink-0 font-bold text-xs text-left">
                      {i + 1}.
                    </div>
                    <div className="text-xs text-gray-600 dark:text-gray-400 leading-snug text-left">
                      {rec}
                    </div>
                  </div>
                ))}
                ...
                {["Critical", "Important", "Nice-to-Have"].map((priority) =>
                  normalizedGrouped[priority]?.length ? (
                    <div key={priority}>
                      ...
                      <span className="ml-1">
                        {normalizedGrouped[priority].join(", ")}
                      </span>
                    </div>
                  ) : null
                )}
...
function SkillColumn({
  ...
  return (
    <div>
      <p className={`font-semibold mb-2 text-${color}-600 dark:text-${color}-400`}>
        {title}
      </p>
...
function parseInsightText(raw?: string) {
  ...
  if (recBlock) {
    const parts = recBlock.split(
      /(?:Group by Priority:|Group by:|Skills Relevance to Job Description:)/i
    );
